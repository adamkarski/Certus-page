  ...
  function analyzeImageColors(img) {
    ...
    for (let section = 0; section < sections; section++) {
      ...
      for (
        let y = startY;
        y < endY;
        y += Math.max(1, Math.floor((endY - startY) / 8))
      ) {
        // Lewa strona - próbkuj od lewej krawędzi widocznej części
        for (
          let x = leftEdgeX;
          x < leftEdgeX + sampleWidth && x < canvas.width;
          x += 2
        ) {
          const index = (y * canvas.width + x) * 4;
          if (data[index + 3] > 128) {
            // Sprawdź czy piksel nie jest przezroczysty
...
>
  ...
  {#if !isMobile}
    ...
    {#each leftGradients as leftGrad, index}
      <div
        class="left-gradient-section"
        style="background: {leftGrad.gradient}; top: {leftGrad.top}; height: {leftGrad.height}"
      ></div>
    {/each}

    {#each rightGradients as rightGrad, index}
      <div
        class="right-gradient-section"
        style="background: {rightGrad.gradient}; top: {rightGrad.top}; height: {rightGrad.height}"
      ></div>
    {/each}
  ...
  <img
    class="gradient"
    src="/assets/landing_targi2025/background1.png"
    alt="Background"
  />
</div>
